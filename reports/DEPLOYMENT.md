...
| 地址类型 | URL | 状态 | 说明 |
|---|---|---|---|
| **立即可用（已在线）** | https://function-stars-wan-boss.trycloudflare.com | ✅ 已实测 HTTPS 200（10/10 页面） | Cloudflare 快速隧道，指向本机 `127.0.0.1:3000`。**需要这台电脑开机并保持服务运行**；重启服务会更换地址 |
| 本地 | http://127.0.0.1:3000 | ✅ 200 | `start-acca-master.cmd` 一键启动 |
| **永久公网地址** | 一键部署（约 3 分钟） | ⏳ 需要一次性账号授权 | 见第 4 节（唯一阻塞点） |

> 隧道说明：本机网络存在代理环境，长连接偶发中断；当前隧道已改用 `--protocol http2 --edge-ip-version 4`，稳定性良好（连续两轮 9/10、10/10 页面检查全部 200）。若隧道断开，双击 `start-public-tunnel.cmd` 重新获取地址（脚本会自动加上述参数）。
